...
namespace flutter_inappwebview_plugin
{
  ...
  void InAppWebViewManager::createInAppWebView(const flutter::EncodableMap* arguments, std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result)
  {
    ...
    if (keepAliveId.has_value() && map_contains(keepAliveWebViews, keepAliveId.value())) {
      // CentroidX: taking a kept-alive view back. Its WebView2 already has
      // the window it was created against, and that window is what the
      // InAppWebView destructor destroys, so the new platform view is handed
      // that one rather than a fresh CreateWindowEx. Upstream creates a
      // window before this branch regardless and nothing ever destroys it:
      // one hidden window (a USER handle) leaked per take-back, and the HMI
      // takes a view back every time an operator returns to a page.
      ...
      if (webView && webView->webViewController) {
        failedLog(webView->webViewController->get_ParentWindow(&existingHwnd));
      }
