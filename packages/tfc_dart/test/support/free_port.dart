...
/// literal.
///
/// Ported verbatim from `tfc_relay_local/test/support/free_port.dart` — the
/// same rig, in the package that now needs it. The reasoning below was written
/// about *this* package's anti-pattern in the first place.
///
/// **The obvious thing is forbidden and the second-obvious thing does not
/// work.** `test/subscription_inactivity_test.dart:30-31` allocates a random
/// offset (`Random().nextInt(1000)`) above a 14840 base, plus a per-test index;
/// that collides across parallel worktrees exactly the way `tfc_dart`'s
/// hard-coded 15432 does (project memory), and the collision surfaces as a
/// fault-mode failure in the code under test rather than as a bind error.
/// `15499` and `15432` are the two port literals this package already carries;
/// nothing in `test/support/` may add a third — not even quoted in a comment,
/// which is why the anti-pattern above is spelled out in prose rather than
/// copied as an expression.
///
