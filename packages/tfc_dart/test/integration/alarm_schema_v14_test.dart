...
/// the four things schema v14 changes are **invisible on SQLite**:
///
...
///   preference JSON — a Postgres database created from that definition
///   refuses every `alarm_history` insert with SQLSTATE 23503. Measured on a
///   scratch database, 2026-09-23. Plants already in the field predate the
///   reference and do not carry the constraint, which is why history has been
///   written there all along; the fix is for the next database created, and
///   arm 1 is what keeps the definition and the plant from diverging again. A
///   SQLite test passes on either schema and proves nothing about either.
...
/// (`onUpgrade(6, 14)`). They are different code paths that must agree, and only
...
/// would let half of v14 be deleted without a red run.
///
/// ## Isolation
///
...
const String upgradedSubject = 'upgraded from v6 (onUpgrade 6 -> 14)';
...
final Map<String, String> databaseNames = <String, String>{
  createdSubject: 'alarm_v14_created_$runSuffix',
  upgradedSubject: 'alarm_v14_upgraded_$runSuffix',
};
...
DatabaseConfig configFor(String database) {
  ...
  return DatabaseConfig(
    ...
    applicationName: 'alarm_schema_v14_test',
  );
}
...
Future<int> insertHistory(
  ...
  final rows = await c.execute(
    ...
    parameters: <String, Object?>{
      ...
      'title': pg.TypedValue(pg.Type.text, 'Chiller 1 over temperature'),
...
/// because the migration does: the v7 access arm runs
...
void main() {
  group('alarm_history schema v14, against a real Postgres', () {
    setUpAll(() async {
      ...
      for (final subject in subjects) {
        expect(migrationFailures[subject], isNull,
            reason: 'the migration for the "$subject" subject THREW: '
                ...
                'about the shape v14 produces, and none of them can mean '
      ...
      group(subject, () {
        ...
        test('arm 1: alarm_history carries no foreign key to alarm', () async {
          ...
          expect(found, isEmpty,
              reason: 'alarm_history still references the alarm table through '
                  ...
                  'preference JSON), so a database created from this '
                  'definition refuses every history insert with 23503 — which '
                  'is what the next plant provisioned would get.');
        });
        ...
        test('arm 2: an insert for an alarm uid no alarm row names succeeds',
          ...
          expect(error, isNull,
              reason: 'the insert was refused: ${describe(error)} '
                  ...
                  'fails on Postgres, invisible only because no plant has '
                  'alarms configured.');
        ...
        test(
          ...
          List<Variable> bind(Variable deactivated) => <Variable>[
                ...
                Variable.withString('Chiller 1 over temperature'),
    ...
    group('the upgrade itself', () {
      test(
          'arm 3: a v6-shaped Postgres database reaches v14 — FK gone, three '
        ...
        // 14: this arm is the alarm-history change, and main's schema was
        // at 13 when it landed. A station that has run every earlier arm gets
        // this one on its next open, which is the safe direction.
        expect(await readDriftMarker(c), 14,
            reason: 'the upgraded database did not end at schema version 14. '
                'It was stamped 6 and opened with the real AppDatabase, so '
                'either schemaVersion is not yet 14 or onUpgrade threw.');

        final fks = await foreignKeysToAlarm(c);
        expect(fks, isEmpty,
            reason: 'the v14 Postgres arm left ${fks.join(', ')} in place. '
      ...
      test(
        ...
        expect(open, isNotNull,
            reason: 'the index was added to the v14 migration arm but not to '
      ...
      test(
        ...
        expect(upgraded, created,
            reason: 'one release produced two shapes for one table. A station '
                ...
                '  created:  $created\n'
                '  upgraded: $upgraded\n'
                'This arm exists because the first draft of the v14 Postgres '
