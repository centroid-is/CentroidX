/// The `config_item` table, declared where a process without a GPU, a PLC
/// stack or a Flutter binding can still read it.
///
...
///
/// So the [Table] subclass lives here, in a library whose only import is
/// drift's pure-Dart core. `AppDatabase` imports the class from this file and
...
/// [$ConfigItemTableTable] in this FFI-free library.
///
/// It is never instantiated and never opened. `AppDatabase` remains the only
/// thing that creates or migrates `config_item`; this class exists purely to
/// give the generator a database to hang the accessor off, and the accessor is
/// then pointed at a real [GeneratedDatabase] by the reads in `page_rows.dart`.
@DriftDatabase(tables: [ConfigItemTable])
