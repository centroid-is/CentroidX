/// A replay stream that tears itself down once the last listener leaves.
///
...
/// constructs one and names nothing else from that library, so this single
/// type tied the collector, and every HMI asset that reaches it through
/// `page_creator/assets/common.dart`, to `dart:ffi`.
///
...
/// monitored item, because after a session loss the server reissues monIds
/// from 1 and an interleaved create/delete can have one key's stale delete
/// destroy another key's fresh item. That ordering is the fix for a real fault
/// and it needs the raw subscription handle, not a stream view.
///
/// While both lived in one library those were private fields and the coupling
/// was invisible. Moving the class makes it explicit rather than new: they are
