...
/// `package:open62541`. dart2js refuses to compile `dart:ffi` at all, so that
/// one import decided whether anything naming a key mapping could be built for
/// a browser. And nearly everything does: every HMI asset reaches
/// `page_creator/assets/common.dart`, and that named `StateManConfig`.
///
/// But the *types* are meaningful on any platform. A conveyor asset names
/// [StateMan] to subscribe; a key mapping is a key mapping whether it was read
/// from a station database or handed to a page renderer in a test. So
/// [StateMan] is declared here as an interface with no implementation, and
/// `OpcUaStateMan` in `state_man.dart` is the implementation that holds real
/// sessions. `GuardedStateMan` wraps either. A widget must never test which it
/// has.
///
/// Nothing here may import `package:open62541/open62541.dart` (use
/// `open62541_types.dart`), `package:drift/native.dart`, or `state_man.dart`.
/// `test/web/web_closure_guard_test.dart` in the root package is the arm that
/// says so.
library;
...
class StateManConfig {
  ...
  // `fromFile`, `fromPrefs` and `toPrefs` are in
  // `state_man_config_storage.dart`, as an extension on this class. They are
  // the only part of this config that needs a filesystem or a drift database,
  // and this file has to stay nameable from a build that has neither.
...
///
/// `OpcUaStateMan` in `state_man.dart` is the implementation that holds real
/// OPC UA, Modbus and M2400 sessions; `GuardedStateMan` wraps one with access
/// control in front. A widget names this type and never learns which it holds.
///
/// **`clients` is deliberately absent.** A `List<ClientWrapper>` is a list of
/// live OPC UA sessions, and `ClientWrapper` is `dart:ffi` — naming it on this
/// interface would put the whole FFI barrel back into the closure of every
/// file that names [StateMan], which is the entire page editor. The two
/// screens that genuinely need live sessions (the browse dialog and the
/// Schneider field-description pane) reach them through
/// `lib/core/opcua_sessions.dart`, behind an io/web seam that says what it
/// does without one.
abstract interface class StateMan {
  ...
  ///
  /// A static on the interface rather than on the OPC UA implementation:
  /// `modbus_device_client.dart` calls it and holds no OPC UA client, and it
  /// is arithmetic on a value — nothing about it needs a session.
  ...
  /// `$variable` substitution — a template is a property of the page, not of
  /// the plant, so it is resolved on this side whatever the transport.
