...
/// all**; and `lib/theme.dart` names `'dejavu-sans'` as the theme's family. Without both
    ...
    for (final (name, theme) in [('light', light), ('dark', dark)]) {
      testWidgets('the account actions menu, open, $name', (tester) async {
        await withClock(Clock.fixed(_frozen), () async {
          ...
          final users = [
            for (final u in _users())
              // By hand, [_navigating]'s reason: the roster speaks
              // [UserSummary], which has no `copyWith`, and carries the
              // decoded page set rather than the column.
              u.username == 'linar'
                  ? UserSummary(
                      username: u.username,
                      roleName: u.roleName,
                      ...
                      homePage: '/packing',
                      alarmAutoNavigate: true,
                      allowedPages: const {'/', '/packing', '/halls/freezer'},
