...
class AssetRegistry {
  ...
  ///
  /// A page row carries each asset as `asset_name: "BpmConfig"`, and this
  /// registry used to find the factory by comparing that string with
  /// `factory.key.toString()`. On a station that is the class name and it works.
  /// A release web build is compiled by dart2js, which **minifies type names**:
  /// `BpmConfig.toString()` there is a couple of letters, no stored name ever
  /// matches, every asset falls to the unrecognized path, and every plant page
  /// draws as an empty canvas — silently, because the logger prints nothing in a
  /// release build.
  ///
  /// A [Type] is still a fine map key — its identity survives minification, only
  /// its spelling does not — so the factories stay keyed by type and this table
  /// pins the spelling beside them. It is the one place the stored names are
  /// written down, and they are literals on purpose: they are the plant's
  ...
  ///
  /// Used in both directions: [parse] resolves a stored name to a factory
  /// through it, and `BaseAsset` takes its own `asset_name` from it when
  /// constructed by a default factory — so what a browser writes into a page is
  /// what a station reads back.
  ...
  static Type? _typeNamed(String name) => (_typesByName ??= {
  ...
  /// and a minified token in a web build; a registration that has to work in a
  /// browser passes the name. The built-in types are in [_names].
        ...
        if (jsonPart.containsKey(constAssetName)) {
          ...
          if (factory != null) {
            try {
              ...
              // The logger prints nothing in a release build, and a browser
              // has no log file: this line is the only trace.
          ...
          // page with nothing on the console — once per name, on the console
          // that is there.
          if (_reportedUnrecognized.add(assetName)) {
            debugPrint('AssetRegistry: unrecognized asset type "$assetName" '
                '— it will not render. This build knows '
                '${_names.length} asset names and none of them is that one.');
          }
        }
  ...
  static Asset? createDefaultAssetByName(String assetName) {
    // Through the name table, for the same reason [parse] is — dart2js
    // minifies `Type.toString()`, so this loop matched nothing in a release
    // web build and every propose_asset call returned null.
    final type = _typeNamed(assetName);
