...
- Options: (1) GPO root push; Chrome/Edge ≥133 `CACertificatesWithConstraints` with `permitted_cidrs` (e.g. <plant-address>/24). (2) Real domain + DNS-01 ACME to private IP — Let's Encrypt explicitly OK (2026-02 dns-persist-01 post); costs CT-logged internal hostnames + internet for renewal; only zero-client-install option. (3) Public CA for private IP/.local banned since 2016; Caddy `tls internal` = just a private CA; mkcert dead.
- **Serve web bundle same-origin as WS**: converts silent wss failure into clickable page-load interstitial; Chrome cert exception is per-HOST across schemes/ports (7-day expiry, debugging aid only); enables SameSite=Strict. https page can never open ws://; <plant-subnet> gets no localhost exemption.
