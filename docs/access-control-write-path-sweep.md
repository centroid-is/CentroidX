...
| `packages/tfc_dart/lib/core/access/access_template_store.dart` | `required AppDatabase db`, `final AppDatabase _db` | `AppDatabase` | `AccessTemplateStore` | `correct as-is` — the handle 2.2's guard delegates through; the check happens above it. Same `_db` spelling as `HistoryViewStore`, so no ninth accessor |
...
| `packages/tfc_dart/lib/core/relay/backend_config_history.dart:48` | `required AppDatabase? database` | `AppDatabase` | `BackendConfigHistory` | `correct as-is` — the gateway's half of `configHistory.*`, and **read-only by construction**: three members, all delegating to `ConfigChangeStore`, none of which can write. It names `AppDatabase` because it builds that store; it issues no statement of its own. The gate is `_PolicyConfigHistory`, through the decorator, at `configure` — the group `kRaisedRoutes[kConfigHistoryRoute]` already demands, so the wire and a station cannot disagree about who may read the log. A null database refuses every member **by name** rather than answering an empty history, because an empty history is indistinguishable from a plant nobody has ever configured |
| `lib/providers/report.dart:20, 91` | `McpDatabase? _storeDb`, `McpDatabase? _engineDb` | `McpDatabase` | `reportStoreProvider`, `reportEngineProvider` | `correct as-is` — the identity-keyed caches that stop a `databaseProvider` rebuild handing every downstream provider a fresh store. Neither writes: the store's two saves go through `GuardedReportStore`, whose group is `configure` and whose row writer is the shared preference store. The gateway branch added on 2026-09-20 holds **no database at all** — it binds the store's read and write seams to `preferencesProvider`, so a relayed save lands on the same `config_item` rows through the door that is already graded and audited |
| `packages/tfc_plant_sim/lib/src/plant.dart` | the simulated plant's own node table | — | `tfc_plant_sim` | `correct as-is` — a **test double**, not a plant surface: the fake OPC UA/Modbus plant the relay lanes drive, in a package no shipped binary depends on (`pubspec.yaml` has it under `dev_dependencies` only). It holds no `config_item`, no `audit_entry` and no access session; what it records is actuations, which is the instrument a read-back cannot be. On this table because the sweep is a text search and the package name contains one of its patterns |
| `packages/tfc_dart/lib/core/relay/backend_shared_preferences.dart` | `database.db`, then `readSharedPreferenceValue(...)` per read | the shared `config_item` preference rows, **read-only** | the acquisition backend's one preference source, from `bin/main.dart` | `correct as-is, and read-only by construction` — added 2026-09-13 with the main merge. `Preferences.create(db:)` stopped reading anything from its database when #465 retired `flutter_preferences`, so the backend would have answered null to `alarm_man_config` and evaluated no alarms at all. Every setter here throws rather than writing: the backend is not an author of the plant's configuration, and a process with one boot read and no reconcile cannot tell "empty" from "not yet migrated". It reaches no guard because there is nothing here to guard — see the library header for the write gap this leaves open |
| `packages/tfc_dart/lib/core/relay/backend_composition.dart` | the `Preferences prefs` it composes the relay's preference service over | the shared preference rows, through the store above | `composeBackendRelay`, from `bin/main.dart` | `session-gated (relay policy)` — this file hands the store to `PreferencesSource`, and every call that reaches it arrived as a JSON-RPC request whose per-key gate and audit row `_PolicyPreferences` applied above it. The same verdict `relayed_preferences.dart` carries from the other end of the same wire |
| `lib/core/relayed_history_views.dart` | `_api.*` on the relayed history-view proxy | **no store in this process** | `historyViewStoreProvider` in gateway mode | `session-gated (relay policy)` — the far end is the enforcement, the same verdict and the same reason as `relayed_access_stores.dart`: these are calls on a wire API, not on an `AppDatabase`, and the gate and the audit row live above the backend's own store |
| `lib/providers/state_man.dart` | `store.keyMappings` off the `ConfigStore` the provider reads | the shared `config_item` key-mapping rows, **read-only** | `stateManProvider` at boot and on the store's change feed | `correct as-is` — a read, and the only one this file makes. The write path it used to hold is gone: `fetchKeyMappings` was deleted with the main merge because it read the retired preference blob and **wrote a two-key example mapping back** when it found nothing, which after #465 would have found null on a fully wired plant. Saves go through `GuardedConfigStore` from the key repository |
| `packages/tfc_dart/lib/core/database_config.dart` | `SecureStorage.getInstance().read/write(key: _configLocation, ...)` | OS keychain | `DatabaseConfig` persistence, from `/advanced/server-config` and `/advanced/preferences` | `route-gated (Phase 2)` — both routes are `administer`; the store itself stays outside the guards, see §3.4. Re-pathed from `packages/tfc_dart/lib/core/database.dart` when the config type was split out of it |
| `packages/tfc_relay_local/lib/src/collect/timescale_sink.dart` | `db.*` inserts into the collected-value hypertables | timeseries tables | the backend's collector, per sample | `out of scope — not authorization data` — plant measurements, written by the acquisition process on its own behalf with no operator in the loop. There is no session to check and no row to attribute; the tables carry no permission and appear on `kRetentionExemptTables` nowhere, because they are the data retention exists for |
| `packages/tfc_relay_local/lib/src/data/timescale_reader.dart` | `db.*` selects over the same hypertables | timeseries tables, **read-only** | the relay's history service, from a panel's chart | `out of scope — not authorization data`, and read-only besides. The same reasoning as the sink above; what a panel may *see* is the route gate's question, not this reader's |
...
| File and line | Call | Store | Reached from | Verdict |
|---|---|---|---|---|
| `lib/pages/key_mappings_file_io.dart:30` | `file.writeAsString(json)` | filesystem | key-mapping export, `/advanced/key-repository` | `route-gated (Phase 2)` — `configure`. Moved out of `key_repository.dart:1886` on 2026-09-13: the export's first line was `Platform.isWindows`, which *throws* under dart2js rather than answering false, so the button was an `UnsupportedError` on a web client. The native arm is the same code at a new address; the web arm hands the browser a download and touches no filesystem, which is why only this file carries the row |
...
| `lib/providers/page_manager.dart` | `relayed.replace(kinds: {page, asset}, …)` | **shared configuration store, over the relay** | the page editor's save on a panel with no mirror | `guarded at the far end` — the gateway grades it at `page_editor_data`, the same `kConfigWriteKeys` row a direct save is checked under, and records one audit row for it. Nothing here decides permission; the binding exists because `writeItems` is final on `PageManager` and the relayed arm has no `GuardedConfigStore` to bind |
| `packages/tfc_dart/lib/core/relay/backend_config_writer.dart` | `ConfigStore.writeItems(...)` | **shared configuration store** | every relayed `preferences.*` mutator and `configItems.replace` | `correct as-is` — the gateway's own writer, and the one place a relayed write reaches Postgres. It decides no permission: the `AccessGroup` gate is `PolicyStateMan`'s and the audit row is the decorator's. What it owns is attribution — `who`, `role_name` and `station` come from the verified identity and from nothing a payload can reach (D-11) |
| `packages/tfc_relay_protocol/lib/src/config_items_api.dart` | `ConfigItemsApi.replace` | **shared configuration store, over the relay** | the wire declaration | `correct as-is` — a shape, not a write. `configWriteKeyByKindSet` lives here because the policy derives a write's grading key from the caller's kind set and `tfc_relay_server` cannot import `tfc_dart`; the two copies are pinned against each other by `config_write_keys_test.dart` |
| `packages/tfc_stateman_contract/lib/testing/fake_access_services.dart` | `configItems.replace` | in-memory fake | the cross-implementation contract | `test-only` — a dev dependency, and the same standing caveat the six rows above carry: promoting `tfc_stateman_contract` out of dev would make this row wrong |
...
| `packages/tfc_dart/lib/core/state_man_config_storage.dart:45` | `prefs.setString(configKey, ..., secret: true, saveToDb: false)` | secure store | `StateManConfig.fromPrefs` at boot when the key is absent | `guarded by 03-06` — routed through `systemWrites`. Moved out of `state_man.dart:63` on 2026-09-13: `StateManConfigStorage` sat beside `OpcUaStateMan`, so naming it linked `dart:ffi` and the Server Config page could not be built for a browser. The write, its flags and its verdict are unchanged |
| `packages/tfc_dart/lib/core/state_man_config_storage.dart:53` | `prefs.setString(configKey, ...)` | secure store | `StateManConfig.toPrefs`, behind a control | `guarded by 03-06` — moved out of `state_man.dart:71` on 2026-09-13, see the row above |
...
| `centroid-hmi/lib/main_web.dart` | `createDeviceLocalPreferences()` at boot | the browser's per-origin device-local store — the transport row and the panel's own settings | the web entry point, once, before `runApp` | `not widget-reachable` — a composition root, not a surface. It opens the store every other web row writes THROUGH; it writes nothing of its own. Added 2026-09-17 with the browser client. The one failure it has already caused is the opposite of a write path: skipping the init left `localPreferencesProvider` throwing a StateError that Riverpod held as a provider error, and the page rendered a blank canvas with no console output |
| `lib/core/relayed_config_items.dart:255` | `_cache.setString(...)` | the device-local cache of the plant's `config_item` rows — pages, assets, key mappings | `RelayedConfigItems`, from the gateway client's boot and on each config change the gateway announces | `session-gated (relay policy)` — every row it caches came from `configItems.items`, which `PolicyStateMan` refuses to a session that does not clear `plantReadFloor`. The cache is a copy of what the gateway already agreed to serve this identity, written device-locally so a browser has a mirror at all; it cannot contain a row the session was not allowed to read |
| `lib/core/secure_storage/browser.dart:29` | `write({key, value})` | nothing — held in memory for the life of the tab | the secure-storage seam on web, reached by `stateManProvider`'s default `state_man_config` write | `correct as-is, and writes nothing durable` — the browser's keychain is deliberately none. The gateway keeps the secrets, the wire has no `secret:` parameter, and `state_man_config` reaches a gateway client redacted. Persisting here would be a "secure" store on a laptop that guards nothing |
| `lib/providers/device_local_store_open_io.dart` | `openDeviceLocalStore(...)` | the device-local rows at `ConfigScope.local` | the `_io` half of the device-local seam, from `initDeviceLocalPreferences` | `not widget-reachable` — a seam, not a surface: it opens the store and hands back a handle. Every row written through it is recorded by the rows for the writers themselves |
| `lib/providers/device_local_store_open_web.dart` | `openDeviceLocalStore(...)` | the browser's per-origin store, and no database | the `_web` half of the same seam | `not widget-reachable` — as above. It answers with no `AppDatabase` at all, which is what makes `kHasDeviceLocalMirror` false in a tab and why the no-mirror branches exist |
| `packages/tfc_dart/lib/core/relay/backend_config_items.dart` | `readSharedConfigItems(...)` per read | the plant's `config_item` rows, **read-only** | `ConfigItemsApi` on the relay backend, from `composeBackendRelay` | `session-gated (relay policy)` — the fifth access family, refused by `PolicyStateMan` unless the session clears `plantReadFloor`, one kind per call. Read-only by construction: there is no writer here, for `backend_shared_preferences.dart`'s reason — the backend is not an author of the plant's configuration |
...
| `packages/tfc_dart/lib/core/state_man_config_storage.dart:45` (`StateManConfig.fromPrefs`) | `state_man_config` | `administer` | 03-06 |
