...
| `packages/tfc_dart/lib/core/config/key_mapping_migration.dart:301, 314` | `db.into(db.configItemTable).insert(...)`, `db.into(db.configChangeTable).insert(...)` | `config_item` and `config_change` — the **shared** rows in Postgres | `migrateKeyMappingsBlobToRows(Database)`, the one-shot blob→rows migration added by milestone v1.2 plan 02-03. **Nothing reaches it at all yet**: it has no caller, and plan 02-05 wires it into the remote-attach path, which runs before `runApp` | `not widget-reachable` — a boot-path data migration, not a surface. It performs no access check and writes no `audit_entry` row, and neither is an omission: it runs before any session exists, on every station, from the attach path. Its rows say so rather than pretending otherwise — `updated_by`/`who` are the literal `'migration'` and `role_name` is `'system'`, so the change rows it appends are attributable to the cutover rather than to a person who was not there. Three things a reader must not conclude: the whole copy is inside **one** transaction whose first statement is `pg_try_advisory_xact_lock`, so two stations booting together cannot both write these rows; the `AppDatabase` in these two statements is a **parameter**, not a handle the file holds, which is what the §4 hits on this file are; and it never writes, rewrites or deletes `flutter_preferences.key_mappings` — the blob stays put as rollback insurance until Phase 4 |
