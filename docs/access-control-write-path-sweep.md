...
| `packages/tfc_dart/lib/core/state_man_config_storage.dart:43` | `prefs.setString(configKey, ..., secret: true, saveToDb: false)` | secure store | `StateManConfig.fromPrefs` at boot when the key is absent | `guarded by 03-06` — routed through `systemWrites`. Moved here from `state_man.dart:442` when `state_man.dart` was split for the web build (the OPC UA client stayed behind, the config storage came out); the write and its guard are unchanged |
| `packages/tfc_dart/lib/core/state_man_config_storage.dart:51` | `prefs.setString(configKey, ...)` | secure store | `StateManConfig.toPrefs`, behind a control | `guarded by 03-06` — moved from `state_man.dart:450` in the same split |
