...
| `packages/tfc_plant_sim/lib/src/plant.dart:160` | `running._server.delete()` | — | `FakePlant.close()`, tearing down the fake plant's OPC UA servers | `test-kit only (dev dependency)` — not a store, and the same broad-grep false positive as the three rows above: this `delete()` frees an open62541 server handle through FFI, which is the teardown order that keeps a shutdown from SEGVing the VM. The package is a test bench (`packages/tfc_plant_sim`): it stands fake PLCs up from a written spec, no shipped binary depends on it, and it holds no database, no preferences and no configuration store. Its only persistence is the spec file a developer wrote by hand |
