...
| `lib/providers/lid_inspection.dart:17` | `DatabaseLidInspectionStore(db.db)` — `customSelect(...)` only, in `packages/tfc_dart/lib/core/lid_inspection.dart` | reads of the service-owned `lid_inspection` table | the Lid inspection asset's pane | `left open: read permissions are deferred` — the store has no write method; the pane's writes are tag writes through `writeTag` |
